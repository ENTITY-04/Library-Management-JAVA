...
public class Receipt_V1 {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(paymentMethod.equalsIgnoreCase("Cash") || paymentMethod.equalsIgnoreCase("Cashs")) {
				break;
			}
		...
		for (int index = 0; index < items.size()-1; index++) {
		    ...
		    //%-12s is for the padding % for printf, -12 for size of padding, s for string in printf
		    System.out.printf("%-12s *%-2d RM%-2.2f ~ RM%-2.2f%n", items.get(index), qtys.get(index), prices.get(index), totalPrice);
		    subTotal += totalPrice;
		}
